...
export class ReaderEngine extends HTMLElement {
  ...
  applySettings(settings, styleText, theme) {
    ...
    // None and narrow use the available width with only the page gutter.
    // A character-count cap would turn narrow into wide margins on a
    // large screen. Normal and wide retain bounded line lengths; gutter
    // changes alone are absorbed by Readium growing the column to match.
    const lineLength = {
      none: { optimalLineLength: 90, minimalLineLength: null, maximalLineLength: null },
      narrow: { optimalLineLength: 90, minimalLineLength: null, maximalLineLength: null },
