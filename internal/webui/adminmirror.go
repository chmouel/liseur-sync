...
//
// The peer password is turned into the kosync key here and the password
// itself is never stored. A save with the mirror enabled is tested
// against the peer first, so a credential that cannot work is refused
// rather than written and left to fail quietly at the next restart.
//
...
func (s *Server) handleSaveMirror(w http.ResponseWriter, r *http.Request, a store.AuthSession, u *store.User) {
	...
	if m.Enabled {
		...
		defer cancel()
		if err := mirror.NewClient(m).Authorize(ctx); err != nil {
