...
//
// Neither is a rearrangement of the catalog alone. `resolveEntityTx`
// turns an observed name into a series, so a shelf changed only here is
// changed back by the next pass that observes the old name. Both
// operations therefore leave a binding behind, and `resolveSeriesTx`
// reads it before it reads the name.
...
func (s *Store) MergeSeries(
	...
	err = tx.QueryRowContext(ctx,
		`SELECT name, normalized_name FROM series WHERE id = ?`,
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE book_series SET series_id = ? WHERE series_id = ?`,
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE book_series_override_items SET series_id = ? WHERE series_id = ?`,
		intoID, seriesID); err != nil {
		return "", err
	}
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE series_bindings SET series_id = ? WHERE series_id = ?`,
		intoID, seriesID); err != nil {
		return "", err
	}
	if err := bindSeriesTx(
	...
	// forgets a nickname.
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM series WHERE id = ?`, seriesID); err != nil {
		return "", err
	}
...
func (s *Store) SplitSeriesFolder(
	...
	var taken string
	err = tx.QueryRowContext(ctx,
		`SELECT id FROM series WHERE normalized_name = ?`, normalized).Scan(&taken)
	switch {
	case err == nil:
	...
	newID := store.NewID()
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO series (id, name, normalized_name, created_at)
		 VALUES (?, ?, ?, ?)`,
		newID, name, normalized, formatTime(at.UTC())); err != nil {
		return "", err
	}
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE book_series SET series_id = ? WHERE series_id = ? AND folder_id = ?`,
		newID, seriesID, folderID); err != nil {
		return "", err
	}
	if _, err := tx.ExecContext(ctx,
		`UPDATE book_series_override_items SET series_id = ?
...
func (s *Store) SeriesBindings(
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT b.id, COALESCE(b.folder_id, ''), COALESCE(f.name, ''),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.ID, &b.FolderID, &b.FolderName, &b.Name,
			&b.NormalizedName, &b.SeriesID, &created, &b.CreatedBy); err != nil {
			return nil, err
		}
...
func (s *Store) DeleteSeriesBinding(ctx context.Context, bindingID string) error {
	res, err := s.db.ExecContext(ctx,
		`DELETE FROM series_bindings WHERE id = ?`, bindingID)
	if err != nil {
		return err
	}
...
func (s *Store) SeriesFolders(
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT bs.folder_id, f.name, COUNT(*)
		   FROM book_series bs
