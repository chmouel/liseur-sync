...
// nameColumns says where a kind's display name comes from. A series
// resolves through the reader's rename layers (ADR-0020), which the
// series prefix already brings along; every other kind has one name,
// the one a scan observed.
	...
	if series {
		...
		if after != nil {
			...
			args = append(args, seriesCursorKey(after),
				formatTime(after.CreatedAt), after.ID)
		}
	} else if after != nil {
