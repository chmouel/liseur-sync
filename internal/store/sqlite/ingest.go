...
func scanIngestJob(row interface{ Scan(...any) error }) (store.IngestJob, error) {
	...
	var clientKey, promotionFingerprint, contentSHA, stagingPath, sourcePath sql.NullString
	var bookID, errorCode, errorDetail, expires sql.NullString
	var artifactsExpired, cleanupPending int
	var created, updated string
	err := row.Scan(
		&job.ID, &job.UserID, &job.LibraryID, &job.QuotaUserID,
		&job.Source, &job.OriginalFilename, &job.Storage, &clientKey, &job.RequestFingerprint,
	...
	job.PromotionFingerprint = assign(promotionFingerprint)
	job.ArtifactsExpired = artifactsExpired != 0
	job.ArtifactCleanupPending = cleanupPending != 0
...
func (s *Store) CreateIngestJob(
	...
	res, err := tx.ExecContext(ctx,
		...
		nullStr(request.ClientKey), request.RequestFingerprint,
		nullStr(request.SourceRelativePath),
...
func (s *Store) ListIngestRecoveryJobs(
	...
	query := `SELECT ` + ingestJobColumns + `
		...
		WHERE j.state IN ('staged', 'validated', 'extracted')
		  AND j.artifacts_expired = 0
		  AND j.updated_at <= ?`
	args := []any{formatIngestTime(before)}
	if after != nil {
		cursorTime := formatIngestTime(after.UpdatedAt)
...
func (s *Store) TransitionIngestJob(
	...
	res, err := tx.ExecContext(ctx,
		...
		nullStr(next.StagingPath), next.ExtractedEmbeddedMetadataJSON,
		nullStr(next.ErrorCode), nullStr(next.ErrorDetail), next.RetryCount,
		next.Revision, formatIngestTime(next.UpdatedAt),
...
func (s *Store) CommitIngestStage(
	...
	if _, err := tx.ExecContext(ctx,
		...
		request.Artifact.SizeBytes, formatIngestTime(request.UpdatedAt)); err != nil {
		if isUniqueErr(err) {
	...
	res, err := tx.ExecContext(ctx,
		...
		formatIngestTime(request.UpdatedAt), userID, jobID, request.ExpectedRevision)
	if err != nil {
		return store.CommitIngestStageResult{}, err
	}
	n, err := res.RowsAffected()
...
func scanBookFileInto(
	...
	var coverPath, coverSHA sql.NullString
	var created, updated string
	targets := []any{
		...
		&coverPath, &coverSHA, &created, &updated,
	}
	if root != nil {
	...
	if partialMD5.Valid {
		file.PartialMD5 = &partialMD5.String
	}
	if dcIdentifier.Valid {
		file.DCIdentifier = &dcIdentifier.String
	}
	if coverPath.Valid && coverPath.String != "" {
		file.CoverRelativePath = &coverPath.String
	}
	file.CoverSHA256 = coverSHA.String
	if file.CreatedAt, err = parseTime(created); err != nil {
		return file, err
	}
	file.UpdatedAt, err = parseTime(updated)
	return file, err
}
...
func promotionReplayTx(
	...
	file, err := scanBookFile(tx.QueryRowContext(ctx,
...
func (s *Store) CommitNewBookPromotion(
	...
	if _, err := tx.ExecContext(ctx,
		...
		formatIngestTime(request.UpdatedAt)); err != nil {
		return store.IngestPromotionResult{}, err
	}
	var blobBytes int64
	if err := tx.QueryRowContext(ctx,
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT OR IGNORE INTO blob_reservations
	...
	res, err := tx.ExecContext(ctx,
		...
		formatIngestTime(request.UpdatedAt),
...
func (s *Store) PurgeExpiredIngestArtifacts(
	...
	rows, err := tx.QueryContext(ctx,
		`SELECT `+ingestJobColumns+`
		 FROM ingest_jobs j
		 WHERE j.artifact_cleanup_pending = 1
		    OR (j.artifacts_expired = 0
	...
	for i, job := range jobs {
		...
		res, err := tx.ExecContext(ctx,
			`UPDATE ingest_jobs
			 SET artifacts_expired = 1, artifact_cleanup_pending = 1,
			     revision = revision + 1
			 WHERE id = ? AND user_id = ? AND state = ? AND revision = ?
			   AND expires_at IS NOT NULL AND expires_at <= ?
			   AND staging_path IS NOT NULL AND artifacts_expired = 0
			   AND artifact_cleanup_pending = 0`,
			job.ID, job.UserID, string(job.State), job.Revision,
			formatIngestTime(before))
		if err != nil {
			return nil, err
		}
		n, err := res.RowsAffected()
...
func (s *Store) CompleteIngestArtifactCleanup(
	...
	res, err := s.db.ExecContext(ctx,
		`UPDATE ingest_jobs
		 SET bytes_received = 0, content_sha256 = NULL, staging_path = NULL,
		     artifact_cleanup_pending = 0, revision = revision + 1
		 WHERE id = ? AND staging_path = ?
		   AND artifacts_expired = 1 AND artifact_cleanup_pending = 1`,
		jobID, stagingPath)
	if err != nil {
		return err
	}
	...
	if expired != 0 && pending == 0 && !currentPath.Valid {
		return nil
	}
