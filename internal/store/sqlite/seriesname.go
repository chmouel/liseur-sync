...
// Series renames (ADR-0020). A rename is a display layer, in the same
// two scopes a claim uses. `series.name` and `series.normalized_name`
// keep meaning "what the last pass observed" and stay the only thing
// resolveEntityTx matches an observed name against.
...
func (s *Store) SetSeriesName(
	...
	if _, err := tx.ExecContext(ctx,
		...
		seriesID, scopeUser, name, normalized, formatTime(at.UTC()), userID,
	); err != nil {
		return err
	}
	return tx.Commit()
...
// with its key. It trims, because a leading space is a typo rather than
// a name, and refuses a name that normalizes away: a series called " "
...
// renameViewer is whose view a collision is checked against. A personal
// rename must not collide with what its author sees; a shared one is
// checked against what a reader with no rename of their own sees, which
// is the shared layer over the scanned names.
//
// A shared rename can therefore still collide with somebody's personal
// one. That reader sees their own name for their own series and the new
// shared name on another, which is a duplicate on their screen and
// nowhere else — not worth a refusal that one reader could impose on
// everybody.
