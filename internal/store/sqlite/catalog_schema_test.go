...
func TestSplitMovesMissingCatalogMapping(t *testing.T) {
	...
	if _, err := s.db.ExecContext(ctx,
		`INSERT INTO blobs (sha256, size_bytes, created_at) VALUES (?, 1, ?)`,
		"book-sha", formatTime(now)); err != nil {
		t.Fatal(err)
	}
	if _, err := s.db.ExecContext(ctx,
		`INSERT INTO book_files
		 ...
		 VALUES ('file1', ?, ?, 'cas', 'book-sha', 1, 'book-sha',
		         'scanned', 'missing.epub', 'missing', ?, ?)`,
		library.ID, book.ID, formatTime(now), formatTime(now)); err != nil {
		t.Fatal(err)
	}
