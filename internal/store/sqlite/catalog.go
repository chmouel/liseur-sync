...
func scanCatalogBook(row interface{ Scan(...any) error }) (store.CatalogBook, error) {
	...
	if b.UpdatedAt, err = parseTime(upAt); err != nil {
		return store.CatalogBook{}, err
	}
	if b.SeenAt, err = parseTimePtr(seenAt); err != nil {
		return store.CatalogBook{}, err
	}
	if b.AbsentAt, err = parseTimePtr(absentAt); err != nil {
		return store.CatalogBook{}, err
	}
	return b, nil
}
...
func (s *Store) ResolveCatalogBookWork(
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO user_book_works
		 (user_id, folder_id, book_id, work_id, created_at)
		 VALUES (?, ?, ?, ?, ?)
		 ON CONFLICT (user_id, book_id) DO NOTHING`,
		userID, folderID, bookID, result.WorkID, formatTime(at)); err != nil {
		return store.WorkResolution{}, err
	}
	...
	var existing string
	if err := tx.QueryRowContext(ctx, `SELECT work_id FROM user_book_works WHERE user_id = ? AND book_id = ?`,
...
// inArgs builds an IN list. Ids are always bound, never interpolated.
