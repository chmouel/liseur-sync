...
func (s *Store) CommitInPlaceBook(
	...
	if _, err := tx.ExecContext(ctx,
		...
		nullStr(file.SourceRelativePath), file.OriginalFilename, mediaType,
		nullStr(file.PartialMD5), nullStr(file.DCIdentifier),
		string(file.Availability), formatTime(request.UpdatedAt),
		formatTime(*file.SourceModifiedAt), formatTime(file.CreatedAt),
		formatTime(file.UpdatedAt)); err != nil {
		if isUniqueErr(err) {
