...
func (s *Store) SessionsEndedBefore(ctx context.Context, userID string, before time.Time, limit int) ([]store.Session, error) {
	...
	args := []any{userID, formatTime(before)}
	if limit > 0 {
		query += ` LIMIT ?`
...
func currentAccountTimezone(ctx context.Context, tx *sql.Tx, userID string) (string, error) {
	var tz string
	err := tx.QueryRowContext(ctx, `SELECT COALESCE(NULLIF(timezone, ''), 'UTC') FROM users WHERE id = ?`, userID).Scan(&tz)
	if errors.Is(err, sql.ErrNoRows) {
...
func preloadEditionPageCounts(ctx context.Context, tx *sql.Tx, userID string, shas []string) (map[string]sql.NullInt64, error) {
	...
	query := `SELECT sha256, page_count FROM editions WHERE user_id = ? AND sha256 IN (` + strings.Join(placeholders, ",") + `)`
