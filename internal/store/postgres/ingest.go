...
func (s *Store) CommitNewBookPromotion(
	...
	if _, err := tx.ExecContext(ctx, q(
		...
		newFile.SourceRelativePath, newFile.OriginalFilename,
		mediaType, newFile.PartialMD5, newFile.DCIdentifier,
		string(newFile.Availability), newFile.CreatedAt.UTC(),
		newFile.UpdatedAt.UTC()); err != nil {
		if isUniqueErr(err) {
